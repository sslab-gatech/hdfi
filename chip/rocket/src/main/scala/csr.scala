...
class CSRFile extends CoreModule
{
  ...
  // TAGGED MEMORY
  /*  Differentiate whether the exception from the pipeline stages is a DFI exception (which is generated 
      from a mismatch between the loaded DFI tag and the expected tag value contained in the instruction) */
  val non_dfi_xcpt = Bool()
  val dfi_xcpt = Bool()
  non_dfi_xcpt := io.exception && io.cause != Causes.dfi_mismatch
  dfi_xcpt := io.exception && io.cause === Causes.dfi_mismatch

  io.fatc := insn_sfence_vm
  // io.evec := Mux(dfi_xcpt, io.pc + 4, /* io.pc is the PC where the exception is generated. For DFI exception
  //                                        let's just move to the next instruction */
  //            Mux(non_dfi_xcpt || csr_xcpt, (reg_mstatus.prv << 6) + MTVEC,
  ...
  when (io.exception || csr_xcpt) {
    printf("io.exception: %x, csr_xcpt: %x, io.cause: %x\n", io.exception, csr_xcpt, io.cause)
    // when (non_dfi_xcpt || csr_xcpt) {
    ...
    // }.otherwise {
    //   // TAGGED MEMORY
    //   printf("This is a DFI exception!\n")
