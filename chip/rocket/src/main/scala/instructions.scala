...
object Instructions {
  ...
  // For DFI tagged memory, add four new instructions:
  //    - ldchk0: load data + DFI tag and check whether that tag value is 0. If not, then
  //              this instruction raises an exception for DFI mismatch
  def LDCHK0             = BitPat("b?????????????????000?????1010111")
  //    - ldchk1: load data + DFI tag and check whether that tag value is 1. If not, then
  //              this instruction raises an exception for DFI mismatch
  def LDCHK1             = BitPat("b?????????????????001?????1010111")
  //    - sdset0: store data + DFI tag value of 0. By default, a normal store instruction
  //              clears the DFI bit associated with the data, so this instruction is
  //              currently redundant
  def SDSET0             = BitPat("b?????????????????010?????1010111")
  //    - sdset1: store data + DFI tag value of 1.
  def SDSET1             = BitPat("b?????????????????011?????1010111")

  // 14th bit (bit 13) determines whether this DFi instruction is load or store.
  // 13th bit (bit 12) is the DFI tag bit value to be stored in memory on store or compared
  // with on load

  // However, if 15th bit (bit 14) is set, then this is the special memcpy instruction, 
  // where the data and DFI tag is loaded from one memory address and stored into another 
  // memory address, with the DFI tag value preserved. A load in this instruction should 
  // be checked against a DFI tag value.
  def SDPRSV            = BitPat("b?????????????????101?????1010111")
  def MEMCPY            = BitPat("b?????????????????100?????1010111")
}
