...
static bool hasCPICPSFunctions(Module &M) {
  for (Module::iterator It = M.begin(), Ie = M.end(); It != Ie; ++It) {
    if (It->hasFnAttribute("cps") || It->hasFnAttribute("cpi"))
      return true;
  }
  return false;
}

static void runCPICPSPasses(Module &M, TargetMachine *TM) {
  initializeAnalysis(*PassRegistry::getPassRegistry());

  PassManager passes;
  passes.add(createVerifierPass());

  passes.add(new DataLayout(*TM->getDataLayout()));
  passes.add(new TargetLibraryInfo(Triple(TM->getTargetTriple())));

#if 1
  TM->addAnalysisPasses(passes);

  // Run all LTO optimizations first, hoping to minimize the amount of
  // instrumentation we'll end up inserting
  if (!DisableOpt) {
...
bool LTOCodeGenerator::generateObjectFile(raw_ostream &out,
  ...
  if (hasCPICPSFunctions(*mergedModule))
    runCPICPSPasses(*mergedModule, _target);
