...
llvm::MDNode *CodeGenModule::getTBAARegionInfo(uint8_t R) {
  // ignore when TBAA is enabled
  if (TBAA)
    return 0;
  if (R)
    return llvm::MDNode::get(getLLVMContext(), llvm::MDString::get(getLLVMContext(), "vregion1"));
  else
    return llvm::MDNode::get(getLLVMContext(), llvm::MDString::get(getLLVMContext(), "vregion0"));
}
