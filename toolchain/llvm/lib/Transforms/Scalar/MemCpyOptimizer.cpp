...
Instruction *MemCpyOpt::tryMergingIntoMemset(Instruction *StartInst,
  ...
  for (MemsetRanges::const_iterator I = Ranges.begin(), E = Ranges.end();
    ...
    bool HasTBAATags = false;
    SmallVector<Value*, 16> TBAATags;
    for (unsigned i = 0, e = Range.TheStores.size(); i != e; ++i) {
      TBAATags.push_back(const_cast<MDNode*>(
                Range.TheStores[i]->getMetadata(LLVMContext::MD_tbaa)));
      if (TBAATags.back() != NULL)
        HasTBAATags |= true;
    }
    if (HasTBAATags)
      AMemSet->setMetadata("tbaa.items",
                           MDNode::get(Builder.getContext(), TBAATags));
