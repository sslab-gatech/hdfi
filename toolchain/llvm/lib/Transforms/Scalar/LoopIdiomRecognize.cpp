...
processLoopStridedStore(Value *DestPtr, unsigned StoreSize,
  ...
  if (MDNode *TBAATag = TheStore->getMetadata(LLVMContext::MD_tbaa))
    NewCall->setMetadata("tbaa.items",
                         MDNode::get(TheStore->getContext(), TBAATag));
...
processLoopStoreOfLoopLoad(StoreInst *SI, unsigned StoreSize,
  ...
  if (MDNode *TBAATag = SI->getMetadata(LLVMContext::MD_tbaa))
    NewCall->setMetadata("tbaa.items", MDNode::get(SI->getContext(), TBAATag));
