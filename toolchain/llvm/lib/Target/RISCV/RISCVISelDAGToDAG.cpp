...
class RISCVDAGToDAGISel : public SelectionDAGISel {
  ...
  bool replaceLoadStore(MachineInstr& MI) {

    if (MI.getOpcode() == RISCV::LD) {
      // check TBAA
      for (MachineInstr::mmo_iterator i = MI.memoperands_begin(),
           e = MI.memoperands_end(); i != e; ++i) {
          const MDNode *TBAA = (*i)->getTBAAInfo();
          if (TBAA) {
            if (MDString *S = dyn_cast_or_null<MDString>(TBAA->getOperand(0))) {
              const TargetInstrInfo &TII = *TM.getInstrInfo();
              if (S->getString().equals("vregion1"))
                MI.setDesc(TII.get(RISCV::LDCHK1));
              else if (S->getString().equals("vregion0"))
                MI.setDesc(TII.get(RISCV::LDCHK0));
            }
          }
      }
    } else if (MI.getOpcode() == RISCV::SD) {
      // check TBAA
      for (MachineInstr::mmo_iterator i = MI.memoperands_begin(),
           e = MI.memoperands_end(); i != e; ++i) {
          const MDNode *TBAA = (*i)->getTBAAInfo();
          if (TBAA) {
            if (MDString *S = dyn_cast_or_null<MDString>(TBAA->getOperand(0))) {
              const TargetInstrInfo &TII = *TM.getInstrInfo();
              if (S->getString().equals("vregion1"))
                MI.setDesc(TII.get(RISCV::SDSET1));
              else if (S->getString().equals("vregion0"))
                MI.setDesc(TII.get(RISCV::SDSET0));
            }
          }
      }
    }
