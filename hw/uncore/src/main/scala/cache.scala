...
case object HasL2Cache extends Field[Boolean]
...
// TAGGED MEMORY
// class LoadGen(typ: UInt, addr: UInt, dat: UInt, zero: Bool) {
class DFILoadGen(typ: UInt, addr: UInt, dat: UInt, zero: Bool, dfiWidth: Int) {
  val t = new StoreGen(typ, addr, dat)
  val sign = typ === MT_B || typ === MT_H || typ === MT_W || typ === MT_D
  ...
  // TAGGED MEMORY
  // get dfi bits
  val dfi = dat(63+dfiWidth,64)
}


class DFIAMOALU extends CacheModule {
  ...
  val io = new Bundle {
    ...
    val dfi = Bits(INPUT, 1)
    val out = Bits(OUTPUT, operandBits+1)
  }
  ...
  io.out := Cat(io.dfi,(wmask & out | ~wmask & io.lhs)(63,0))
}
...
class L2DataArray(delay: Int) extends L2HellaCacheModule {
  ...
  //TAGGED MEMOTY
  println(f"\trowBits in L2:\t ${rowBits}%d")
/*
  when(io.resp.fire()){
    printf("io.resp.fire() in L2DataArray\n")
    printf("io.resp.bits.data:\t%x\n",io.resp.bits.data)
  }
  */
  val array = SeqMem(Vec(Bits(width=8), (128/8)), nWays*nSets*refillCycles)

  val array_flags = Reg(init=Vec.fill(nWays*nSets*refillCycles)(Bits(0,width=2)))
  ...
  //TAGGED MEMORY
  val wdata_in_dummy = io.write.bits.data(127,0)
  val wdata = Vec.tabulate(rowBits/8)(i => wdata_in_dummy(8*(i+1)-1,8*i)  )
/*
  when(io.write.valid){
    printf("io.write.valid in L2DataArray\n")
    printf("io.write.bits.data:\t%x\n",io.write.bits.data)
    for(i <- 0 until 16){
        printf("%x",wdata(i))
    }
    printf("\n")
...
*/
  //TAGGED MEMORY
  val wmask_dfi = io.write.bits.wmask
  val wmask = wmask_dfi.toBools

  when (io.write.valid) { 
    array.write(waddr, wdata, wmask)
    array_flags(waddr) := io.write.bits.data(129,128)
  }

  val r_req = Pipe(io.read.fire(), io.read.bits)
  val flag_out = RegEnable(array_flags(raddr),ren)
  io.resp := Pipe(r_req.valid, r_req.bits, delay)
  //TAGGED MEMORY
  io.resp.bits.data := Pipe(r_req.valid, Cat(flag_out,array.read(raddr, ren).toBits), delay).bits
...
/*
  ...
  when(r_req.valid){
    printf("r_req.valid=1\n")
    printf("\traddr:\t%x\n",raddr)
    printf("\tarray_flags:\t%x\n",array_flags(raddr))
    printf("\tarray.read:\t%x\n",array.read(raddr, ren).toBits)
    printf("\tren:\t%x\n",ren)
    printf("\tflag_out:\t%x\n",flag_out)
  }
*/
...
/*
  when(io.inner.acquire.fire()){
    printf("io.inner.acquire fires in L2HellaCacheBank\n")
    printf("\tid:\t%x\n",io.inner.acquire.bits.client_xact_id)
    printf("\tunion:\t%x\n",io.inner.acquire.bits.union)
    printf("\ta_type:\t%x\n",io.inner.acquire.bits.a_type)
  }

  when(io.outer.acquire.fire()){
    printf("io.outer.acquire fires in L2HellaCacheBank\n")
    printf("\tid:\t%x\n",io.outer.acquire.bits.client_xact_id)
    printf("\tunion:\t%x\n",io.outer.acquire.bits.union)
    printf("\ta_type:\t%x\n",io.outer.acquire.bits.a_type)
  }
  
  when(io.inner.grant.fire()){
    printf("io.inner.grant.fire() in L2HellaCache\n")
    printf("\tdata:\t%x\n",io.inner.grant.bits.data)
    printf("\tid:\t%x\n",io.inner.grant.bits.client_xact_id)
  }
 */ 
...
class L2AcquireTracker(trackerId: Int) extends L2XactTracker {
  ...
  //TAGGED MEMORY
    ...
    val wmask = Cat(UInt(0,width=2),FillInterleaved(8, wmask_buffer(beat)))
/*
    printf("temp\n")
    printf("\twmask:\t%x\n",wmask)
    printf("\tnew_data:\t%x\n",data_buffer(beat))
    printf("\told_data:\t%x\n",old_data)
    printf("\tbeat:\t%x\n",beat)
    printf("\tdata to be written to the buffer:\t%x\n", ~wmask & old_data |
...
*/
  ...
  //TAGGED MEMORY
      op_code = xact.union(M_SZ,1)),//op_code()),
    Bundle(Acquire(xact))(outerTLParams))
  io.oacq().data := data_buffer(oacq_data_idx)
/*
  when(io.outer.acquire.fire()){
    printf("io.outer.acquire fires in L2AcquireTracker\n")
    printf("\tid:\t%x\n",io.outer.acquire.bits.client_xact_id)
    printf("\tunion:\t%x\n",io.outer.acquire.bits.union)
    printf("\ta_type:\t%x\n",io.outer.acquire.bits.a_type)
  }
...
/*
  when(io.inner.grant.fire()){
    printf("inner.grant.fire() in L2 Acquire Tracker\n")
    printf("\tdata:\t%x\n",io.inner.grant.bits.data)
  }
*/
  ...
  when(io.inner.acquire.fire() && io.iacq().hasData()) { // state <= s_meta_wrtie
    ...
    //val full = Cat(UInt(0,width=2),FillInterleaved(8, wmask))
    //val full = Cat(UInt(3,width=2),FillInterleaved(8, wmask))

    //val full = FillInterleaved(8, Cat(UInt(1,width=1),wmask))
    val full = FillInterleaved(8, wmask)
/*
    printf("temp2\n")
    printf("\tfull:\t%x\n",full)
    printf("\ttemp_data:\t%x\n",io.iacq().data)
    printf("\tdata to be written to the buffer2:\t%x\n", (~full & data_buffer(beat)) | (full & io.iacq().data))
*/
