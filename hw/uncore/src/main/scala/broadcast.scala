...
class L2BroadcastHub extends ManagerCoherenceAgent
  ...
  println(f"\t# Transactors of L2BroadCastHub:\t$nTransactors%x")
  println(f"\t# Release Transactors of L2BroadCastHub:\t$nReleaseTransactors%x")
...
class BroadcastVoluntaryReleaseTracker(trackerId: Int) extends BroadcastXactTracker {
  ...
  println(f"\tReleaseTracker #$trackerId%x generated")
...
class BroadcastAcquireTracker(trackerId: Int) extends BroadcastXactTracker {
  ...
  println(f"\tAcquireTracker #$trackerId%x generated")
  println(f"\tinner tlClientXactIdBits in AcquireTracker:\t${io.inner.acquire.bits.tlClientXactIdBits}%x")
  println(f"\touter tlClientXactIdBits in AcquireTracker:\t${io.outer.acquire.bits.tlClientXactIdBits}%x")
...
//TAGGED MEMORY
  val oacq_read_block = GetBlock(
    client_xact_id = UInt(trackerId),
    addr_block = xact.addr_block,
    m_op = xact.union(M_SZ,1)
    )
  //printf("m_op in broadcast:\t%x\n",xact.union(M_SZ,1))
  io.outer.acquire.valid := Bool(false)
